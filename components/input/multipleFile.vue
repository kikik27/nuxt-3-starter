...
<template>
  <div></div>
  <!-- <div class="relative my-3 h-full w-full first:mt-0 last:mb-0" :class="{ success: meta.valid }">
    <label v-if="label" :for="name" class="mb-2 block" :class="{
      ...
      <label :class="{
        ...
        <div v-if="!!log_file?.length"
          ...
          <div class="relative grid w-full grid-cols-1 gap-2 px-2">
            <div v-for="(item, index) in log_file" :key="index" :class="{ 'border-red-500 ': !!errorMessage }"
              ...
              <div
                ...
                <label
                  ...
                  <input :id="name" :name="name" type="file" class="hidden" :accept="accept" @change="changeFile" />
                </label>
                <button type="button"
                  class="grow-0 rounded-lg bg-white p-2 text-red-500 shadow-md hover:bg-red-500 hover:text-white"
                  @click="removeFile(index)">
                  <Icon name="material-symbols:delete-outline-rounded" class="relative h-5 w-5" />
                </button>
              </div>
            </div>
          </div>
        ...
        <div v-if="!!!log_file?.length"
          ...
          <div class="flex h-[94px] flex-col items-center justify-start gap-3 self-stretch">
            <div
              class="inline-flex h-10 w-10 items-center justify-center rounded-lg border border-gray-200 bg-white p-2.5 shadow">
              <Icon name="ph:cloud-arrow-up-duotone" class="relative flex h-5 w-5 flex-col items-start justify-start" />
            </div>
            <div class="flex h-[42px] flex-col items-center justify-start gap-1 self-stretch">
              <div class="inline-flex items-start justify-center gap-1 self-stretch">
              ...
              <div class="self-stretch text-center text-xs font-normal leading-[18px] text-slate-600">
                {{ comment }}
              </div>
            </div>
          </div>
        </div>
        <input v-if="!!!log_file?.length && !disabled" :id="name" :name="name" type="file" class="hidden" :accept="accept"
          @change="changeFile" />
      </label>
    </div>
    <div v-if="!!errorMessage" class="absolute -bottom-5 right-0 mt-1 text-xs text-red-500 dark:text-red-500">
      {{ errorMessage }}
    </div>
  </div> -->
</template>